...
\section*{Profile}
\begin{description}
\item\textbf{Platform/DevOps leader with 10+ years building and operating scalable cloud platforms (GCP, Kubernetes, Terraform).} Experienced in driving DevSecOps transformations—policy-as-code, secrets management, and IaC scanning—while delivering measurable reductions in onboarding time and infrastructure costs and maintaining high uptime and strong security practices. \\
\end{description}
...
\parbox{\linewidth}{\raggedright
  \textbf{AI \& Automation:} Gemini, MCP server\\[ -9.5pt]
  \textbf{CI / CD:} ArgoCD, GitHub Actions, Jenkins\\[ -9.5pt]
  \textbf{Cloud Platforms:} Google Cloud, Windows Azure\\[ -9.5pt]
  \textbf{Cloud Security:} Vault, Wiz, XDR\\[ -9.5pt]
  \textbf{Container Orchestration:} Docker, Kubernetes, VMware Tanzu\\[ -9.5pt]
  \textbf{DevSecOps:} HashiCorp Vault, IaC scanning, Sentinel/OPA, secrets management\\[ -9.5pt]
  \textbf{Enterprise Workload Orchestration:} VMware vSphere\\
}
\end{flushleft}
\begin{flushright}
\parbox{\linewidth}{\raggedleft
  \textbf{GitOps:} Argocd, Fluxcd\\[ -9.5pt]
  \textbf{Infrastructure as Code:} Ansible, Terraform\\[ -9.5pt]
  \textbf{Monitoring Tools:} Grafana, Prometheus\\[ -9.5pt]
  \textbf{Observability \& Tracing:} OpenTelemetry\\[ -9.5pt]
  \textbf{Programming Languages:} Bash, Golang, Python\\[ -9.5pt]
  \textbf{Service Mesh:} Istio, Linkerd\\[ -9.5pt]
  \textbf{Version Control:} CVS, Git (Github.com), SVN\\
}
\end{flushright}
\end{multicols}
